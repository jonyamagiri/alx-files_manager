...
// Creating two separate queues for file processing and user processing
const fileQueue = new Queue('fileQueue', 'redis://127.0.0.1:6379');
const userQueue = new Queue('userQueue', 'redis://127.0.0.1:6379');

// Function to generate a thumbnail image with a specific width
async function thumbNail(width, localPath) {
  const thumbnail = await imageThumbnail(localPath, { width });
  return thumbnail;
}

// Processing files from the fileQueue
fileQueue.process(async (job, done) => {
  ...
  files.findOne({ _id: idObject }, async (err, file) => {
    if (!file) {
      ...
      // Generating thumbnails with different sizes
      ...
      // Writing the thumbnails to the file system
...
// Processing users from the userQueue
